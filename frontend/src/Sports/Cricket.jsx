...
const Cricket = () => {
  ...
  const fetchMatches = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:3000/matches/cricket")

      console.log(response);

      // Organize matches by status
      const liveMatches = response.data.matches.filter((match) => match.status === "live")
      const upcomingMatches = response.data.matches.filter((match) => match.status === "upcoming")
      const completedMatches = response.data.matches.filter((match) => match.status === "completed")
